...
from common_lib.utils.utils import get_user_info_by_auth,ret_in_json,ret_in_dict
from common_lib.common_func import g_stdlogging
import json

def is_direct_res(info_dict):
    if type(info_dict) == dict:
        if all(k in info_dict for k in ("code", "msg","data")):
            return True
    return False
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                g_stdlogging.exception("[rule_check_ce]%s" % ce)
                # if ce.code == 10400:
                #     the_res = {
                #         "error": "invalid_token",
...
# 请求装饰器
def rule_check_inner(rule_list=[]):
    """
请求装饰器
    :param rule_list: [
        {
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                #     }
                #     return json.dumps(the_res, indent=4, ensure_ascii=False)
                return ret_in_dict(msg=ce.chi_msg,code=ce.code)
            except Exception as e:
                g_stdlogging.exception("[rule_check]%s" % e)
                return ret_in_dict(msg="未知错误",code=-1)
        return wrapper
    return decorator
